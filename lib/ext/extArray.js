...
Array.prototype.vBrief = function () {
  ...
  switch (true) {
    case this.length === 0:
      return _clay.aeu;
    case take >= 1 && take <= this.length - 3:
      return [].concat(_toConsumableArray(this.take(take).tagsIndexed(abstract)), [' .', ' .', ' .'], [this.lastTagIndexed(abstract)]).join(_clay.rn);
    default:
      return this.tagsIndexed(abstract).join(_clay.rn);
  }
};
